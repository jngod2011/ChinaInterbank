...
matrix <- matrix[-c(1:length(2000:2006))]
d.matrix <- d.matrix[-c(1:length(2000:2006))]
                                
matrix.all <- list()
for (i in 1:length(2007:2016)) {
  temp.Ccombas <- group.stockprice[match(l.is.listed[[i]],group.stockprice$Abbr),"Ccombas"]
  temp.locate <- match(temp.Ccombas,colnames(matrix[[i]]))
  print(temp.locate)
  print(i)
  temp.matrix <- matrix[[i]][temp.locate,temp.locate]
  temp.matrix[is.na(temp.matrix)] <- 0
  colnames(temp.matrix) <- l.is.listed[[i]];rownames(temp.matrix) <- l.is.listed[[i]]
  matrix.all[[i]] <- temp.matrix
}

d.matrix.all <- list()
for (i in 1:length(2007:2016)) {
  #print(c(2004:2016)[[i]])
  temp.Cname <- group.stockprice[match(l.is.listed[[i]],group.stockprice$Abbr),"Cname"]
  temp.locate <- match(temp.Cname,colnames(matrix[[i]]))
  #print(temp.locate)
  d.matrix.all[[i]] <- d.matrix[[i]][temp.locate,temp.locate]
}
